...
# Figure 1: Phase 1 manual vs automated test inventory
phase1 = pd.read_csv(ROOT / "phase1" / "metrics" / "phase1-baseline-metrics.csv")
manual = float(phase1.loc[phase1["Metric"] == "Phase 1 manual test case files (total)", "Value"].iloc[0])
...
plt.title("Phase 1 Test Inventory: Manual vs Automated")
plt.ylabel("Count")
save_fig(FIG / "phase1" / "phase1_manual_vs_automated_inventory.png")


# Figure 2: Phase 2 automation coverage by module
...
plt.ylim(0, 100)
plt.title("Phase 2 Automation Coverage by Module")
plt.ylabel("Coverage (%)")
save_fig(FIG / "phase2" / "phase2_automation_coverage_by_module.png")


# Figure 3: Midterm defects vs risk (planned vs actual risk)
mid_risk = pd.read_csv(ROOT / "midterm" / "charts" / "midterm-planned-vs-actual-chart.csv")
...
plt.title("Midterm Planned vs Observed Risk")
plt.ylabel("Risk score")
plt.legend()
save_fig(FIG / "midterm" / "midterm_defects_vs_risk.png")


# Figure 4: Phase 3 performance response times
...
plt.title("Phase 3 Performance Response Times")
plt.ylabel("Latency (ms)")
plt.legend()
save_fig(FIG / "phase3" / "phase3_performance_response_times.png")


# Figure 5: Phase 3 mutation score by module
...
plt.ylim(0, 100)
plt.title("Phase 3 Mutation Score by Module")
plt.ylabel("Mutation score (%)")
save_fig(FIG / "phase3" / "phase3_mutation_score_by_module.png")


# Figure 6: Phase 3 chaos availability by scenario
...
plt.ylim(0, 100)
plt.title("Phase 3 Chaos Availability by Scenario")
...
midterm_qg = pd.read_csv(ROOT / "midterm" / "metrics" / "midterm-quality-gate-evaluation.csv")
...
summary_df = pd.DataFrame(
    {
        "phase": ["Phase 1", "Phase 2", "Midterm", "Phase 3"],
